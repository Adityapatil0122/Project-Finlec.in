...
export async function POST(request: Request) {
  ...
  if (!state) {
    return NextResponse.json({ message: "Unable to create support ticket." }, { status: 404 });
  }

  return NextResponse.json({ message: "Support request created successfully.", tickets: state.supportTickets });
}
